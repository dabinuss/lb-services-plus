...
Category request definitions are data-driven in `shared/request_definitions.lua`. Universal `general` templates provide freely written general requests, appointments, complaints, information requests and callback requests for every company. `specialized` templates expose minimal category-specific fields for pickup, roadside/towing assistance, delivery, table reservations, medical transport, property viewing, legal assistance and police requests. Select options are allow-listed and validated server-side. Phone fields are prefilled from the currently equipped LB Phone number but remain ordinary editable text fields. Add a field definition, template, category mapping, and phase set to introduce another controlled workflow without changing request rendering. Company-specific choices remain stored in `services_plus_company_request_settings`; obsolete or unknown templates, fields and phases are rejected or safely replaced with valid defaults.
