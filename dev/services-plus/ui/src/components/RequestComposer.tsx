...
export function RequestComposer({ company, locale, busy, onClose, onSubmit }: Props) {
  ...
  return <div className="editor-overlay" role="dialog" aria-modal="true" aria-labelledby="request-title"><section className="leader-editor request-editor">
    ...
    {options && <><label className="request-field"><span>{options.label}</span><select value={templateId} onChange={(event) => { const next = event.target.value; setTemplateId(next); setValues(initialValues(options, next)); }}>{generalTemplates.length > 0 && <optgroup label={t(locale, "generalRequests")}>{generalTemplates.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</optgroup>}{specializedTemplates.length > 0 && <optgroup label={t(locale, "specialRequests")}>{specializedTemplates.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</optgroup>}</select></label>
