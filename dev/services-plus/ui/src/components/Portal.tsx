...
interface Props {
  ...
  onNumberOperations: (numbers: NumberOperationsPatch[]) => void; onNumberSubscription: (numberId: string, enabled: boolean) => void;
  ...
  onEligibility: (numberId: string, targetSource: number, enabled: boolean) => void;
...
export function Portal({ user, companies, locale, busy, onEnter, onLeave, onStatus, onDispatch, onCompanySave, onNumberOperations, onNumberSubscription, onCallEmployee, onContactEmployee, workspace, onOpenConversation, onAcceptRequest, onTransitionRequest, onReturnRequest, onSaveRequestSettings, onEligibility, onDeleteRequest, onDeleteConversation }: Props) {
  ...
  return <main className="portal">
    ...
    <section className="control-band">
      ...
      {workspace?.numberSubscriptions && <div className="line-subscriptions"><span className="section-label">{t(locale, "staffedLines")}</span>{workspace.numberSubscriptions.filter((number) => number.enabled).map((number) => <label key={number.numberId}><span><Phone size={15} /><strong>{number.label}</strong></span><input type="checkbox" checked={number.subscribed} disabled={busy || !number.canSubscribe} onChange={(event) => onNumberSubscription(number.numberId, event.target.checked)} /></label>)}</div>}
      {self.isLeader && workspace?.numberEligibility && workspace.numberEligibility.some((number) => workspace.numberSubscriptions?.some((subscription) => subscription.numberId === number.numberId && subscription.staffingMode === "restricted")) && <div className="line-eligibility"><span className="section-label">{t(locale, "restrictedLineAccess")}</span><small>{t(locale, "restrictedLineAccessHint")}</small>{workspace.numberEligibility.filter((number) => workspace.numberSubscriptions?.some((subscription) => subscription.numberId === number.numberId && subscription.staffingMode === "restricted")).map((number) => <section key={number.numberId}><strong>{number.label}</strong>{number.people.map((person) => <label key={person.source}><span>{person.name}</span><input type="checkbox" checked={person.eligible} disabled={busy} onChange={(event) => onEligibility(number.numberId, person.source, event.target.checked)} /></label>)}</section>)}</div>}
    </section>
