...
export function Activity({ data, locale, loading, onConversation, onCancelRequest }: Props) {
  ...
  const requests = (items: typeof active) => <div className="activity-list">{items.map((request) => <article key={request.id}><span className="activity-icon request"><Send size={17} /></span><div><strong>{request.companyName ?? request.companyId}</strong><small>{Object.values(request.payload || {}).filter(Boolean).slice(0, 2).join(" · ") || request.details || t(locale, "request")}</small><em>{request.status}</em></div>{["pending", "active", "returned"].includes(request.status) ? <button type="button" className="activity-cancel" onClick={() => onCancelRequest(request.id)} title={t(locale, "cancel")}><XCircle size={17} /></button> : <span className="status-chip"><Clock3 size={12} />{request.status}</span>}</article>)}{!items.length && <div className="activity-empty"><Send size={21} />{t(locale, "noRequests")}</div>}</div>;
