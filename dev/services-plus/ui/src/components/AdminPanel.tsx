...
export function AdminPanel({ data, locale, busy, onSaveCompany, onDeleteCompany, onRestoreCompany, onSaveSettings, onSaveCategory }: Props) {
  ...
  return <main className="admin-panel">
    ...
    <section className="admin-section"><header><div><span className="eyebrow">Requests</span><h2>{t(locale, "categoryCompetition")}</h2></div><Send size={20} /></header>
      <div className="switch-list category-settings">{data.categories.map((category: Category) => <label key={category.id}><span><strong>{category.names[locale] || category.name}</strong><small>{t(locale, "categoryCompetitionHint")}</small></span><input type="checkbox" checked={category.requestCompetition} disabled={busy} onChange={(event) => onSaveCategory(category.id, event.target.checked)} /></label>)}</div>
    </section>
    ...
    {editing && <div className="editor-overlay" role="dialog" aria-modal="true"><section className="leader-editor admin-editor">
      ...
      <div className="switch-list compact"><label><span><Send size={17} /><span><strong>{t(locale, "requests")}</strong>{!requestTemplatesAvailable && <small>{t(locale, "noSpecialRequests")}</small>}</span></span><input type="checkbox" checked={editing.requestsEnabled && requestTemplatesAvailable} disabled={!requestTemplatesAvailable} onChange={(event) => field("requestsEnabled", event.target.checked)} /></label><label><span><Building2 size={17} /><strong>{t(locale, "messages")}</strong></span><input type="checkbox" checked={editing.messagesEnabled} onChange={(event) => field("messagesEnabled", event.target.checked)} /></label><label><span><BellRing size={17} /><span><strong>{t(locale, "requestNotificationActionable")}</strong><small>{t(locale, "requestNotificationActionableHint")}</small></span></span><input type="checkbox" checked={editing.requestsEnabled && requestTemplatesAvailable && editing.requestNotificationActionable} disabled={!requestTemplatesAvailable || !editing.requestsEnabled} onChange={(event) => field("requestNotificationActionable", event.target.checked)} /></label></div>
      <div className="number-editor"><header><strong>{t(locale, "phoneNumbers")}</strong><button type="button" className="icon-action" onClick={() => field("numbers", [...editing.numbers, { label: "Main Line", number: "", distribution: "ring_all", sharedInbox: true, enabled: true, callsEnabled: true, inboxEnabled: true, requestsEnabled: true, publicVisible: true }])} aria-label={t(locale, "addNumber")}><Plus size={17} /></button></header>{editing.numbers.map((number, index) => { const update = (patch: Partial<typeof number>) => field("numbers", editing.numbers.map((item, itemIndex) => itemIndex === index ? { ...item, ...patch } : item)); return <div className="number-row" key={index}><label className="number-field"><span>{t(locale, "numberLabel")}</span><input value={number.label} maxLength={80} onChange={(event) => update({ label: event.target.value })} /></label><label className="number-field"><span>{t(locale, "phoneNumber")}</span><input value={number.number} maxLength={32} onChange={(event) => update({ number: event.target.value })} /></label><label className="number-field"><span>{t(locale, "callDistribution")}</span><select value={number.distribution} onChange={(event) => update({ distribution: event.target.value as typeof number.distribution })}><option value="ring_all">{t(locale, "ringAll")}</option><option value="random">{t(locale, "random")}</option><option value="dispatch_only">{t(locale, "dispatchOnly")}</option></select></label><div className="number-capabilities"><strong>{t(locale, "numberCapabilities")}</strong><div className="capability-grid"><label><span>{t(locale, "numberOn")}</span><input type="checkbox" checked={number.enabled} onChange={(event) => update({ enabled: event.target.checked })} /></label><label><span>{t(locale, "calls")}</span><input type="checkbox" checked={number.callsEnabled} onChange={(event) => update({ callsEnabled: event.target.checked })} /></label><label><span>{t(locale, "numberInbox")}</span><input type="checkbox" checked={number.sharedInbox && number.inboxEnabled} onChange={(event) => update({ sharedInbox: event.target.checked, inboxEnabled: event.target.checked })} /></label><label><span>{t(locale, "requests")}</span><input type="checkbox" checked={number.requestsEnabled} onChange={(event) => update({ requestsEnabled: event.target.checked })} /></label><label><span>{t(locale, "public")}</span><input type="checkbox" checked={number.publicVisible} onChange={(event) => update({ publicVisible: event.target.checked })} /></label></div><small className="capability-help">{t(locale, "numberCapabilitiesHint")}</small></div><button type="button" className="delete-icon" onClick={() => field("numbers", editing.numbers.filter((_, itemIndex) => itemIndex !== index))} aria-label="Remove"><Trash2 size={16} /></button></div>; })}</div>
