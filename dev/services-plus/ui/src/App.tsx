...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToNui((message: AppMessage) => {
      ...
      if (message.type === "request.offer") { const item = message.payload as CitizenRequest; const companyId = item.companyId || item.company_id; const company = stateRef.current?.companies.find((candidate) => candidate.id === companyId); const category = stateRef.current?.categories.find((candidate) => candidate.id === company?.categoryId); const values = Object.values(item.payload || {}).filter(Boolean).slice(0, 2).join(" · "); setOffer({ kind: "request", id: item.id, title: item.requestLabel || item.request_label || t(localeRef.current, "incomingRequest"), subtitle: values || t(localeRef.current, "newRequest"), payload: item.payload, companyName: company?.displayName || item.companyName, companyLogo: company?.logo, companyBackground: company?.backgroundImage, categoryName: category?.names[localeRef.current] || company?.categoryName, categoryIcon: category?.icon }); }
      ...
      if (message.type === "request.offer.removed") { const item = message.payload as { id: number }; setOffer((current) => current?.kind === "request" && current.id === item.id ? null : current); }
      if (message.type === "request.updated") { const item = message.payload as CitizenRequest; if (item.status !== "pending" && item.status !== "returned") setOffer((current) => current?.kind === "request" && current.id === item.id ? null : current); }
  ...
  const declineOffer = async () => { if (!offer) return; await run(offer.kind === "call" ? "declineCall" : "declineRequest", { id: offer.id }, () => setOffer(null)); };
