import type { AdminCompany, AdminState, ApiResponse, CitizenRequest, Company, CompanyPatch, CompanyWorkspace, ConversationData, DeletedCompany, InboxConversation, InboxMessage, InitialState, MessageReactionUpdate, MyActivity, RequestSettings, WorkspaceCursor, WorkspaceSection } from "../types";
...
export async function fetchNui<T>(event: string, data: unknown = {}): Promise<ApiResponse<T>> {
  ...
  if (event === "acceptRequest" || event === "returnRequest") {
    const payload = data as { id: number }; const request = browserWorkspace.requests.find((item) => item.id === payload.id);
    if (request) { if (event === "acceptRequest") { request.status = "active"; if (employment?.employee) { employment.employee.status = "busy"; employment.employee.activeRequest = true; employment.employee.activeRequestId = request.id; } } if (event === "returnRequest") { request.status = "returned"; if (employment?.employee) { employment.employee.status = "available"; employment.employee.activeRequest = false; employment.employee.activeRequestId = undefined; } } return { success: true, data: structuredClone(request) as T }; }
  }
...
browserWorkspace.calls.push(...Array.from({ length: 62 }, (_, index) => ({ id: 300 - index, callerNumber: index === 3 ? undefined : `***${2000 + index}`, numberId: "taxi-main", status: index % 5 === 0 ? "missed" : "completed", created_at: new Date(Date.UTC(2026, 6, 31, 18, 0, -index)).toISOString() })));
browserWorkspace.conversations.push(...Array.from({ length: 62 }, (_, index) => ({ id: 500 - index, numberId: index % 2 === 0 ? "taxi-main" : "taxi-booking", numberLabel: index % 2 === 0 ? "Dispatch" : "Bookings", externalNumber: `***${3000 + index}`, lastMessage: `Browser preview conversation ${index + 1}`, lastMessageAt: new Date(Date.UTC(2026, 6, 31, 16, 0, -index)).toISOString(), unreadCount: index % 9 === 0 ? 1 : 0 })));
